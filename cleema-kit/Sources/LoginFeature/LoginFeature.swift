...
public struct LoginView: View {
    ...
    public var body: some View {
        WithViewStore(store) { viewStore in
            SwitchStore(store) {
                CaseLet(state: /Login.State.loggedIn, action: Login.Action.loggedIn) { loggedInStore in
                    AppView(store: loggedInStore)
                        .transition(.opacity)
                }

                CaseLet(state: /Login.State.newUser, action: Login.Action.newUser) { newUserStore in
                    NavigationView {
                ...
                CaseLet(state: /Login.State.authenticate, action: Login.Action.authenticateUser) { _ in
                ...
                CaseLet(state: /Login.State.fetching, action: Login.Action.newUser) { _ in
                    NavigationView {
                        ZStack {
                            ProgressView()
                                .tint(.defaultText)
