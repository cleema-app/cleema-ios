    ...
    public var body: some View {
        GroupBox {
            ZStack(alignment: .topTrailing) {
                    ...
                    VStack(alignment: .leading) {
                        ...
                        HStack(alignment: .bottom) {
                            LabeledContentView(
                                ...
                                value: challenge.partner?.title ?? L10n.Item.Partner.Title.defaultValue
                            )

                            Spacer()

                            if let remoteImage = challenge.partner?.logo {
                ...
                BadgeView(text: challenge.startDate.formatted(date: .numeric, time: .omitted), color: .news)
            }
        }
