        ...
        WithViewStore(store) { viewStore in
            VStack(alignment: .leading, spacing: 12) {
                if viewStore.currentAmount < viewStore.totalAmount {
                    ...
                    SwitchStore(self.store.scope(state: \.step, action: ProjectFunding.Action.steps)) {
                        CaseLet(
                            state: /Steps.State.amount,
                            action: Steps.Action.amount,
                            then: AmountView.init(store:)
                        )

                        CaseLet(state: /Steps.State.pending, action: Steps.Action.pending) { _ in
                            VStack {
                                ProgressView()
